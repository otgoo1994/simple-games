...
import { ComChapterCard } from '~/shared/ui';
import { ChevronRight, ChevronLeft } from 'lucide-react';
import { ChapterType } from '~/entities/common';

interface ComWebtoonChartSwiperProps {
  webtoons: ChapterType[];
  swiperKey: string;
  isLoading: boolean;
}

export const ComWebtoonChartSwiper = ({
  webtoons,
  swiperKey,
  isLoading,
}: ComWebtoonChartSwiperProps) => {
      ...
      >
        {webtoons.map((webtoon) => (
          <SwiperSlide key={`${swiperKey}-${webtoon.seq}`}>
            <ComChapterCard key={webtoon.seq} webtoon={webtoon} />
          </SwiperSlide>
        ))}
      </Swiper>
